...
> **デフォルトプロバイダは `mistral`（モデル: `codestral-latest`）です。**
...
```bash
# VIBE プリセット（Mistral codestral-latest + VIBE モードを自動設定）
export MISTRAL_API_KEY="your-key"
...
| オプション | デフォルト | 説明 |
|-----------|-----------|------|
| `--vibe` | — | Mistral codestral-latest + VIBE モードのプリセット（下記参照） |
| `--provider` | `mistral` | プロバイダ選択 |
| `--model` | `codestral-latest` | モデル名（プロバイダごとに異なる） |
| `--api-key` | — | APIキー（env var 推奨） |
| `--base-url` | — | カスタムエンドポイント |
| `--mode` | `壁打ち` | モード選択 |
| `--persona` | `default` | ペルソナ選択 |
| `--temperature` | `0.4` | サンプリング温度（0〜2） |
...
- `--vibe` 単体: `provider=mistral`・`model=codestral-latest`・`mode=VIBE` を自動設定
- `--vibe` と `--provider anthropic` を同時指定: **明示した `--provider` が優先**
...
- **Web UI バインド**: デフォルトは `127.0.0.1`（ローカルホストのみ）。
...
> Par défaut (`cargo run` sans options) : provider `mistral` + modèle `codestral-latest`.
