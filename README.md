...
LLM は確率的でドリフトしがちです。OBSTRAL は「境界・承認・監査・優先度付け」で実装を前進させます。
...
Coder が mkdir, git init, ファイル作成を順番に実行
  ↓
Rust版: bash コードブロックに ▶ run ボタン（クリックで実行）
Lite版: tool call を抽出 → 承認待ち（Approveで実行）
  ↓
結果がその場に表示
```
...
| ` ```bash` / ` ```sh` / ` ```shell` ブロック | `run_command`（Rust: ▶ run / Lite: 承認→実行） |
...
This is not just a chat client.

#### Concept

LLMs are stochastic. Development needs constraints. OBSTRAL adds boundaries + approvals + adversarial auditing + prioritization to prevent drift.
...
Rust mode: bash/shell code blocks show a **▶ run** button — click to execute locally.

Lite mode: the server extracts tool calls (e.g. `run_command`) and runs them behind approval gates, then prints results inline.
...
| ` ```bash` / ` ```sh` / ` ```shell` | `run_command` (Rust: ▶ run / Lite: approval → run) |
...
Pour les développeurs qui veulent que les LLM *agissent*, pas seulement discuter.

#### Pourquoi OBSTRAL existe

Beaucoup d'outils LLM optimisent la conversation. OBSTRAL optimise des **boucles d'exécution contrôlées**:
- tension dual-agent (Coder vs Observer)
- scoring des propositions
- phase gating
- détection de boucle
- approvals (gates)

Ce n'est pas juste un client de chat.

#### Concept

Les LLM sont stochastiques. Le dev a besoin de contraintes. OBSTRAL ajoute frontières + approvals + audit adversarial + priorisation pour éviter la dérive.
...
# workspace par défaut (Lite): `~/obstral-work`
```
...
Mode Rust: les blocs `bash`/`sh`/`shell` affichent un bouton **▶ run** — cliquez pour exécuter localement.

Mode Lite: le serveur extrait les tool calls (ex: `run_command`) et les exécute derrière des approvals, puis affiche les résultats.
...
| Blocs `bash` / `sh` | `run_command` (Rust: ▶ run / Lite: approbation → exécution) |
