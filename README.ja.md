...
各ページには、eval の prompt 形状、provider/model、再実行コマンド、runtime snapshot、そして green な `runtime_eval` report から取った概算 token 使用量を載せています。

---

## 現在のBenchmark Milestone

いまの milestone は素朴ですが大事です。OBSTRAL は `runtime_eval` から 2 つの小さな repo を end-to-end で再生成し、それらを git に含めず、それでも再現可能な形で結果を記録できるところまで来ています。
...
次に効くのは、もっと派手な demo を足すことより、runtime を長い仕事でも信頼できるものにしていくことです。

- **long-running の耐久性**: session resume はかなり良くなりましたが、数時間単位の仕事には progress file、context compaction、drift に応じた replan がまだ欲しいです。
- **benchmark の幅**: fresh repo scaffold は良い入口で、次は既存 repo の bugfix、小さな web app、観測ループに入りやすい中規模 refactor が効きます。
