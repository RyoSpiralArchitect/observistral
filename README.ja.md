...
- プロンプト省略で再開: `obstral agent --tool-root . --session <path>` をもう一度実行
...
- **CLI（`obstral agent`）**: `exec` とファイル編集（`write_file` / `patch_file` / `apply_diff`）の前に承認プロンプトが出る。`--yes` で自動承認。
- **TUI**: 現状はツールを自動承認。
