...
| 役割 | やること | やらないこと |
|---|---|---|
| **Coder** | 実行 — ファイル操作、シェルコマンド、エージェントループ（最大12回）、4つのツール | 自分のコードを見直すこと |
...
Coderはシェルコマンドだけに限定されない。4つの専用ツールを持っている:
...
`write_file` と `patch_file` はテンポラリファイル → リネームのパターンを使うため、書き込み途中でクラッシュしても破損ファイルが残らない。
...
エージェントの全アクションはワーキングディレクトリ内で実行される。デフォルト: `.tmp/<thread-id>`。

実際のプロジェクトで作業するには `tool_root` をプロジェクトパスに設定する:
- **TUI**: `--tool-root .` フラグ、または実行中に `/root <path>` スラッシュコマンド
