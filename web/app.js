  ...
  function App() {
    ...
    const coderContextPacket = () => {
      ...
      const sendCoder = async (overrideText) => {
        ...
        const wantsMaterial = /(?:repo|repository|scaffold|bootstrap|init|setup|create|generate|implement|install|build|test|run|git|winget|bash|powershell|cmd|command|リポ|リポジトリ|雛形|ひな形|プロジェクト|実装|ファイル|作成|作ろ|作って|生成|追加|組み込|コマンド|実行|インストール|セットアップ|ビルド|テスト)/i.test(text);
